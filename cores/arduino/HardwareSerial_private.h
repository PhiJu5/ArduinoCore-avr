...
// Macros to handle the 9th bit
#define SERIAL9BIT_IDX(x)    ((x)>>3)
#define SERIAL9BIT_POS(x)    ((x)-(((x)>>3)<<3))
#define SERIAL9BIT_sbi(x,y) \
	sbi(x[SERIAL9BIT_IDX(y)], SERIAL9BIT_POS(y))
#define SERIAL9BIT_cbi(x,y) \
	cbi(x[SERIAL9BIT_IDX(y)], SERIAL9BIT_POS(y))
#define SERIAL9BIT_bit(x,y) \
    bit_is_set(x[SERIAL9BIT_IDX(y)], SERIAL9BIT_POS(y))
...
void HardwareSerial::_rx_complete_irq(void)
{
  if (bit_is_clear(*_ucsra, UPE0)) {
    ...
    // room
	bool vbit9 = (bit_is_set(*_ucsrb, UCSZ02) && bit_is_set(*_ucsrb, RXB80)) ? true : false;
    ...
    if (i != _rx_buffer_tail) {
      ...
      if (bit_is_set(*_ucsrb, UCSZ02)) {
        if (vbit9) {
          SERIAL9BIT_sbi(_rx_9bit, _rx_buffer_head);
        } else {
          SERIAL9BIT_cbi(_rx_9bit, _rx_buffer_head);
        }
      }
      _rx_buffer_head = i;
    }
